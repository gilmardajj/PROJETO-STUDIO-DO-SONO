...
function initContactForms() {
  // Roteamento por assunto: cada opção do select usa uma access_key (=inbox) diferente no Web3Forms.
  const FORM_ROUTES = {
    duvida:    { key: 'ab51cc48-1494-4e4e-89be-5a886b870f3e', label: 'Dúvida / Orçamento' },              // sdr@grupostudiomais.com.br
    vendedor:  { key: '528312af-e406-48c4-993b-d0f1e2480cd9', label: 'Vaga - Vendedor', isVaga: true },   // vagas@grupostudiomais.com.br
    gerente:   { key: '528312af-e406-48c4-993b-d0f1e2480cd9', label: 'Vaga - Gerente', isVaga: true },    // vagas@grupostudiomais.com.br
    parceiro:  { key: 'b5a5c233-2b90-4f13-af86-dd558673c739', label: 'Parceiro' }                         // marketing@grupostudiomais.com.br
  };
  const RESUME_PLACEHOLDER = 'Cole aqui as informações do seu currículo.';
  ...
  document.querySelectorAll('[data-contact-form]').forEach(form => {
    ...
    const routeSelect = form.querySelector('[data-routes-email]');
    const messageTextarea = form.querySelector('textarea[name="message"]');
    const resumeHint = form.querySelector('[data-resume-hint]');
    const defaultMessagePlaceholder = messageTextarea ? messageTextarea.placeholder : '';

    function showFeedback(message, type) {
      if (!feedbackEl) return;
      feedbackEl.textContent = message;
      feedbackEl.className = 'text-sm font-medium rounded-xl px-4 py-3 ' +
        (type === 'success'
          ? 'bg-green-50 text-green-700 border border-green-200'
          : 'bg-red-50 text-red-700 border border-red-200');
    }

    // Ao escolher uma vaga, avisa que não dá pra anexar arquivo e pede pra colar o currículo na mensagem.
    if (routeSelect) {
      routeSelect.addEventListener('change', () => {
        const route = FORM_ROUTES[routeSelect.value];
        const isVaga = !!(route && route.isVaga);
        if (resumeHint) resumeHint.classList.toggle('hidden', !isVaga);
        if (messageTextarea) {
          messageTextarea.placeholder = isVaga ? RESUME_PLACEHOLDER : defaultMessagePlaceholder;
        }
      });
    }
    ...
    form.addEventListener('submit', async (e) => {
      ...
      // Escolhe a inbox certa com base no "Assunto/Vaga" selecionado.
      if (routeSelect && accessKeyInput) {
        const route = FORM_ROUTES[routeSelect.value];
        if (!route) {
          showFeedback('Selecione um assunto antes de enviar.', 'error');
          return;
        }
        accessKeyInput.value = route.key;
        if (subjectInput) {
          subjectInput.value = 'Site Studio do Sono - ' + route.label;
        }
      }
      ...
      try {
        ...
        showFeedback('Não foi possível enviar. Tente novamente ou fale pelo WhatsApp.', 'error');
      } finally {
        if (submitBtn) {
...
function initBackToTop() {
  ...
  const SHOW_AFTER = 400; // px rolados antes do botão aparecer
  ...
  // Throttla o scroll com requestAnimationFrame pra não travar o scroll no mobile.
  let ticking = false;
  function onScroll() {
    if (ticking) return;
    ticking = true;
  ...
  onScroll(); // cobre o caso de recarregar a página já rolada
